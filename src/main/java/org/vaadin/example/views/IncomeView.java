...
public class IncomeView extends VerticalLayout {
    ...
    public IncomeView(IncomeService incomeService, NoteService noteService, SessionService sessionService, UserService userService) {
        ...
        HorizontalLayout formLayout = new HorizontalLayout(sourceField, amountField, datePicker, frequencyField, addButton, deleteButton);
        ...
        notesInput.addKeyPressListener(event -> {
    ...
    private void addIncome() {
        ...
        income.setDate(date != null ? java.sql.Date.valueOf(date) : null); // Convert LocalDate to java.sql.Date
    ...
    private void updateIncomeSourcesCard(List<Income> incomes, BigDecimal totalIncome) {
        ...
        if (content.getElement().getChildren().noneMatch(child -> child.getClass().equals("card-title"))) {
            H2 cardTitle = new H2("Income Streams");
            cardTitle.addClassName("card-title");
            content.add(cardTitle);
        }
    
        sourcePercentages.forEach((source, amount) -> {
            @SuppressWarnings("deprecation")
            BigDecimal percentage = amount.multiply(BigDecimal.valueOf(100)).divide(totalIncome, 2, BigDecimal.ROUND_HALF_UP);
            ...
            Paragraph detailsParagraph = new Paragraph("Percentage of Total Income: " + percentage + "%; Amount: $" + amount.toString());
